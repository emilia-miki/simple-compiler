...
parseArgs tokens = (M.fromList $ zip argNames [0..], rest)
  where argNames = map liftArg $ takeWhile (/= TChar ']') $ tail tokens
...
takeParens tokens = reverse $ go tokens 0 []
  where go :: [Token] -> Int -> [Token] -> [Token]
        go (t : ts) 0 [] = case t of
          TChar '(' -> go ts 1 []
          _other -> undefined
